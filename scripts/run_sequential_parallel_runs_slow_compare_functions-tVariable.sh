...
RUN_CMDS+=("$TIME_CMD $NM_EXEC $BASE_COMMAND_F16 -t 1 -l $LOG_DIR/run1.log")
RUN_CMDS+=("$TIME_CMD $NM_EXEC $BASE_COMMAND_F17 -t 1 -l $LOG_DIR/run2.log")
for((i=1; i<9; i++)); do
    let RUNi0=($i * 3)
    let RUNi1=($i * 3 + 1)
    let RUNi2=($i * 3 + 2)
    let T=($i * 2)
    RUN_CMDS+=("$TIME_CMD $NM_EXEC $BASE_COMMAND_F1 -t $T -l $LOG_DIR/run$RUNi0.log")
    RUN_CMDS+=("$TIME_CMD $NM_EXEC $BASE_COMMAND_F16 -t $T -l $LOG_DIR/run$RUNi1.log")
    RUN_CMDS+=("$TIME_CMD $NM_EXEC $BASE_COMMAND_F17 -t $T -l $LOG_DIR/run$RUNi2.log")
done
